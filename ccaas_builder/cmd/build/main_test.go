...
func TestArguements(t *testing.T) {
	...
	// Build ledger binary
	gt := NewWithT(t)
	buildCmd, err := gexec.Build("github.com/hxx258456/fabric/ccaas_builder/cmd/build")
...
func TestGoodPath(t *testing.T) {
	...
	releaseCmd, err := gexec.Build("github.com/hxx258456/fabric/ccaas_builder/cmd/build")
	gt.Expect(err).NotTo(HaveOccurred())
	defer gexec.CleanupBuildArtifacts()
